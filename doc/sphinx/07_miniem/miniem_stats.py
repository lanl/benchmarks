...
"""
This is a self-contained script that computes MiniEM run statistics from a
...
class BuildDocHelp(object):
    ...
    def __init__(self):
        ...
        self.parser.add_argument(
            ...
            type=str,
            default="Size",
            help="header that stores the size information",
        )
    ...
    def _get_data(self):
        ...
        with open(self.file_name, newline="") as csvfile:
            ...
            for row in csvfilereader:
                ...
                if col_size is None:
                    try:
                        col_size = row.index(self.header_size)
                    except ValueError:
                        pass
                if col_ranksperdomain is None:
                    try:
                        ...
                        pass
                if None in (col_fom, col_size, col_ranksperdomain):
                    continue
                if str(row[col_fom]) == self.header_fom:
                    continue

                key = (
                    str(row[col_size]).zfill(3)
                    + "|"
