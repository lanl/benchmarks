...
static IOR_offset_t WriteOrReadBlast(IOR_param_t *, void *, int);
...
#else
	printf("IOR-: MPI Coordinated Test of Parallel I/O\n");
#endif
	printf("\n");
	fflush(stdout);
}
...
static void ShowSetup(IOR_param_t *params)
{
	...
	printf(params->filePerProc ? "file-per-process" : "single-shared-file");
        ...
        if (params->blastMode == FALSE) {
            ...
            printf(" dynamic offsets (BLAST mode)\n");
        }
...
static void ShowTest(IOR_param_t * test)
{
        ...
        fprintf(stdout, "\t%s=%d\n", "threadsPerTask", test->threadsPerTask);
...
#ifdef _OPENMP
                /* set and report threading  */
                if ( params->threadsPerTask > 0 )
                        omp_set_num_threads( params->threadsPerTask );
                if ( params->blastMode ) {
#pragma omp parallel
                        {
#pragma omp single
                                {
                                        fprintf(stdout, "Threads per task:    %d\n", omp_get_num_threads());
                                }
                        }
#endif
                }
...
static void ValidTests(IOR_param_t * test)
{
        ...
        if (test->threadsPerTask > 1 && !test->blastMode)
                WARN_RESET("threadsPerTask only valid for blastMode",
                            test, &defaults, threadsPerTask);
...
static IOR_offset_t WriteOrRead(IOR_param_t * test, void *fd, int access)
{
        ...
        if (test->blastMode) {
            return WriteOrReadBlast( test, fd, access );
        }
...
/*
 *  Write or read data BLAST style.  This spawns threads which 
 *   1. obtain lock on the file
 *   2. read (or write) blockSize bytes sequentially
 *   3. release lock
 *   4. re-read the blocks from #2
 *   5. repeat
 *  BLAST only performs reads, but this function supports writing in this
 *  pattern for consistency with the base WriteOrRead() function
 */
static IOR_offset_t WriteOrReadBlast(IOR_param_t * test, void *fd, int access)
{
        IOR_offset_t transferCount = 0;
        IOR_offset_t dataMoved = 0;     /* for data rate calculation */
        int errors = 0;

        /* rewind the global state of the file */
        test->offset = 0;
#pragma omp parallel
        {
            IOR_offset_t fileSize;
            ...
            IOR_param_t testLocal;
            IOR_offset_t amtXferred;
            IOR_offset_t transfer;
            IOR_offset_t transferCountLocal = 0;
            IOR_offset_t dataMovedLocal = 0;     /* for data rate calculation */
            IOR_offset_t blockBegin, blockEnd;
            int errorsLocal = 0;

            /* initialize values */
            pretendRank = (rank + rankOffset) % test->numTasks;

            /* this changes buffer, checkBuffer, readCheckBuffer */
            XferBuffersSetup(&buffer, &checkBuffer, &readCheckBuffer, access, test, pretendRank);
            ...
            while ( test->offset < fileSize && !hitStonewall ) {
                    /* IOR's backend xfer functions are not thread safe, so we have
                    to work around this.  we create a shallow local copy of the
                    test parameters because the xfer functions rely on 
                    test->offset; fortunately, xfer is pure and doesn't access
                    anything pointed to by elements in IOR_param_t */
                    testLocal = *test;

                    /* first pass - build index.  BLAST triggers page faults as it
                    reads the final byte of every record; assuming records are
                    fewer than 4K bytes (2 bits per sequence, so 16K bp), this is
                    a sequential 4K read/write */
#pragma omp critical 
                    {
                        blockBegin = test->offset;
                        blockEnd = blockBegin + test->blockSize;
                        /* advance the global state (test) */
                        for ( test->offset;
                            test->offset < blockEnd && test->offset < fileSize;
                            test->offset += test->transferSize )
                        {
                            if (test->storeFileOffset == TRUE)
                                FillBuffer(buffer, test, test->offset, pretendRank);
                            transfer = test->transferSize;
                            ...
                            if (access == WRITE) {
                                ...
                                if (amtXferred != transfer) ERR("cannot read from file write check");
                                transferCountLocal++;
                                errorsLocal += CompareBuffers(buffer, checkBuffer, transfer, transferCountLocal, test, WRITECHECK);
                            } else if (access == READCHECK) {
                                ReadCheck(fd, buffer, checkBuffer, readCheckBuffer,
                                        test, transfer, test->blockSize, &amtXferred,
                                        &transferCountLocal, access, &errorsLocal);
                            }
                            dataMovedLocal += amtXferred;
                        }
                    }
                    /* end critical section */

                    /* second pass - use private testLocal instead of public test */
                    for ( testLocal.offset = blockBegin;
                        testLocal.offset < blockEnd && testLocal.offset < fileSize;
                        testLocal.offset += test->transferSize )
                    {
                        if (testLocal.storeFileOffset == TRUE)
                            FillBuffer(buffer, &testLocal, testLocal.offset, pretendRank);
                        /* this critical will cause workers to block on any
                         * other thread doing its first pass, which is how BLAST
                         * behaves 
                         */
                        #pragma omp critical
                        {
                            transfer = testLocal.transferSize;
                        }

                        if (access == WRITE) {
                            amtXferred = backend->xfer(access, fd, buffer, transfer, &testLocal);
                            if (amtXferred != transfer) ERR("cannot write to file");
                        } else if (access == READ) {
                            amtXferred = backend->xfer(access, fd, buffer, transfer, &testLocal);
                            if (amtXferred != transfer) ERR("cannot read from file");
                            ...
                            memset(checkBuffer, 'a', transfer);
                            amtXferred = backend->xfer(access, fd, checkBuffer, transfer, &testLocal);
                            if (amtXferred != transfer) ERR("cannot read from file write check");
                            transferCountLocal++;
                            errorsLocal += CompareBuffers(buffer, checkBuffer, transfer, transferCountLocal, &testLocal, WRITECHECK);
                        } else if (access == READCHECK) {
                            ReadCheck(fd, buffer, checkBuffer, readCheckBuffer,
                                    &testLocal, transfer, testLocal.blockSize, &amtXferred,
                                    &transferCountLocal, access, &errorsLocal);
                        }
                        dataMovedLocal += amtXferred;
                    }
                    ...
                    hitStonewall = ((test->deadlineForStonewalling != 0)
                                    && ((GetTimeStamp() - startForStonewall)
                                        > test->deadlineForStonewalling));
#pragma omp atomic
                    dataMoved += dataMovedLocal;
#pragma omp atomic
                    transferCount += transferCountLocal;
#pragma omp atomic
                    errors += errorsLocal;

                    dataMovedLocal = 0;
                    transferCountLocal = 0;
                    errorsLocal = 0;
            }
            XferBuffersFree(buffer, checkBuffer, readCheckBuffer, access);
        }
