...
#ifndef _WIN32
#include <regex.h>
#if defined(__sun) || defined(__APPLE__)  /* SunOS/OSX do not support statfs(), instead uses statvfs() */
...
#if defined(__sun) || defined(__APPLE__)                    /* SunOS does not support statfs(), instead uses statvfs() */
