...
int main(int argc, char *argv[])
{
    ...
    for (mpi_type_itr = 0; mpi_type_itr < options.omb_dtype_itr;
        ...
        if (0 == myid) {
            MPI_CHECK(MPI_Send_init(s_buf, num_elements, omb_curr_datatype, 1,
                                    1, MPI_COMM_WORLD, &send_obj));
            MPI_CHECK(MPI_Recv_init(r_buf, num_elements, omb_curr_datatype, 1,
                                    1, MPI_COMM_WORLD, &recv_obj));
        } else if (1 == myid) {
            MPI_CHECK(MPI_Recv_init(r_buf, num_elements, omb_curr_datatype, 0,
                                    1, MPI_COMM_WORLD, &recv_obj));
            MPI_CHECK(MPI_Send_init(s_buf, num_elements, omb_curr_datatype, 0,
                                    1, MPI_COMM_WORLD, &send_obj));
        }
