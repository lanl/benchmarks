...
public class OSUScatter extends BenchmarkUtils {
  ...
  public OSUScatter(String[] args) {
    super(args, BenchmarkUtils.OSU_SCATTER_TEST);
    this.args = args;
  }
  ...
  public void runBenchmark() throws Exception {
    ...
    for (size = minMsgSize ; size <= maxMsgSize ; size = 2*size) {
      ...
      for (i = 0 ; i < benchWarmupIters + benchIters ; i++) {
        ...
        if(useBufferAPI) {
          MPI.COMM_WORLD.scatter(sendBuffer, size, MPI.BYTE, 
  ...
  private void runWarmupLoop() throws Exception {
    ...
    for(int i=0 ; i<INITIAL_WARMUP; i++) {
      ...
      if(useBufferAPI) {
        MPI.COMM_WORLD.scatter(sendBuffer, 1024, MPI.BYTE, 
  ...
  protected void printHeader() {
    System.out.println(OSU_SCATTER_TEST);
  ...
  public static void main(String[] args) throws Exception {
    OSUScatter reduceTest = new OSUScatter(args);
