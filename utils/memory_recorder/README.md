...
Set `CXX` and/or `FC` (defaults `g++` and `gfortran`) in your environment or on the command line to point at the correct C++ compiler (C++11 required).
...
To use the modulefile (on LANL systems), copy it to `${HOME}/privatemodules` and load the `use.own` module before loading `memory_recorder.tcl`.
Adjust the `softdir` variable on line 23 of `memory_recorder.tcl` if setting a custom `PREFIX`.

## USAGE

There are two ways to use the program.

1. Set `LD_PRELOAD` to the path to `libmemory_recorder.so`. This will print out the max, min, and average memory usage per node with the nodes where the max and min memory usage occurs.

2. Build the target program, include the `memory_recorder.h` header file in your program where you intend to use it, and link the `libmemory_recorder.so` library. You can then create an instance of the `MemoryRecorder` struct and call the methods to record the memory usage.
...
2. The following are member 
    ...
    * `write_rss(int filewrite)`: If called without an argument or with the default argument, 0, it writes the total RSSMax of the program along with the max and min per node RSSMax values and the nodes where they occur to stdout. If `filewrite` is any other number it
    writes the maximum RSS usage for each process and node to files `rss_{rel_node_number}_{node_name}.meminfo` in the current working dir.
...
In order to write the RSSMax values, call `write_rss()`. No other method calls are necessary to perform this operation.
...
1. The `read_meminfo("LOC")` methods read the meminfo file and store the results in a map with keys ("LOC") "PostMalloc", "PostFill", and "PostFree" at respective points in the code.
2. Takes 1 argument: `int`, the number of elements in each array. Default is `2**26`, resulting in 3 `256 MiB` arrays.
3. The `main()` function creates an instance of the `MemoryRecorder` struct and calls the methods to record the memory usage.
