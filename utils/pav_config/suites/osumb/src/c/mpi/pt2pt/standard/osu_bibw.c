            ...
            for (k = 0; k <= options.warmup_validation; k++) {
                if (myid == 0) {
                    ...
                    for (j = 0; j < window_size; j++) {
                        if (options.buf_num == SINGLE) {
                            MPI_CHECK(MPI_Irecv(
                                r_buf[0], num_elements, omb_curr_datatype, 1,
                                10, MPI_COMM_WORLD, recv_request + j));
                        } else {
                            MPI_CHECK(MPI_Irecv(
                                r_buf[j], num_elements, omb_curr_datatype, 1,
                                10, MPI_COMM_WORLD, recv_request + j));
                        }
                    }
                    ...
                    for (j = 0; j < window_size; j++) {
                        if (options.buf_num == SINGLE) {
                            MPI_CHECK(MPI_Isend(
                                s_buf[0], num_elements, omb_curr_datatype, 1,
                                100, MPI_COMM_WORLD, send_request + j));
                        } else {
                            MPI_CHECK(MPI_Isend(
                                s_buf[j], num_elements, omb_curr_datatype, 1,
                                100, MPI_COMM_WORLD, send_request + j));
                        }
                    }
                    ...
                    for (j = 0; j < window_size; j++) {
                        if (options.buf_num == SINGLE) {
                            MPI_CHECK(MPI_Irecv(
                                r_buf[0], num_elements, omb_curr_datatype, 0,
                                100, MPI_COMM_WORLD, recv_request + j));
                        } else {
                            MPI_CHECK(MPI_Irecv(
                                r_buf[j], num_elements, omb_curr_datatype, 0,
                                100, MPI_COMM_WORLD, recv_request + j));
                        }
                    }
                    ...
                    for (j = 0; j < window_size; j++) {
                        if (options.buf_num == SINGLE) {
                            MPI_CHECK(MPI_Isend(
                                s_buf[0], num_elements, omb_curr_datatype, 0,
                                10, MPI_COMM_WORLD, send_request + j));
                        } else {
                            MPI_CHECK(MPI_Isend(
                                s_buf[j], num_elements, omb_curr_datatype, 0,
                                10, MPI_COMM_WORLD, send_request + j));
                        }
                    }
