...
public class OSUAllgatherv extends BenchmarkUtils {
  ...
  public OSUAllgatherv() {
  }

  public OSUAllgatherv(String[] args) {
    super(args, BenchmarkUtils.OSU_ALLGATHERV_TEST);
    this.args = args;
  }
  ...
  public void runBenchmark() throws Exception {
    ...
    for (size = minMsgSize ; size <= maxMsgSize ; size = 2*size) {
      ...
      for (i = 0 ; i < benchWarmupIters + benchIters ; i++) {
        ...
        if(useBufferAPI) {
          MPI.COMM_WORLD.allGatherv(sendBuffer, size, MPI.BYTE, 
  ...
  private void runWarmupLoop() throws Exception {
    ...
    for(int i=0 ; i<INITIAL_WARMUP; i++) {
      ...
      if(useBufferAPI) {
        MPI.COMM_WORLD.allGatherv(sendBuffer, 1024, MPI.BYTE, 
            recvBuffer, recvcounts, rdispls, MPI.BYTE);
      } else { 
        MPI.COMM_WORLD.allGatherv(sendBuffer, 1024, MPI.BYTE, 
            recvBuffer, recvcounts, rdispls, MPI.BYTE);
  ...
  protected void printHeader() {
    System.out.println(OSU_ALLGATHERV_TEST);
  ...
  public static void main(String[] args) throws Exception {
    OSUAllgatherv allGathervTest = new OSUAllgatherv(args);

    MPI.Init(args); 

    if (MPI.COMM_WORLD.getRank() == 0)
      allGathervTest.printHeader();

    if (allGathervTest.printVersion) {
      if (MPI.COMM_WORLD.getRank() == 0)
        allGathervTest.printVersion();

      MPI.Finalize();
      return;
    }

    if (allGathervTest.printHelp) {
      if (MPI.COMM_WORLD.getRank() == 0)
        allGathervTest.printHelp();
