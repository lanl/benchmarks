...
public class OSUAlltoall extends BenchmarkUtils {
  ...
  public OSUAlltoall(String[] args) {
    super(args, BenchmarkUtils.OSU_ALLTOALL_TEST);
    this.args = args;
  }
  ...
  public void runBenchmark() throws Exception {
    ...
    for (size = minMsgSize ; size <= maxMsgSize ; size = 2*size) {
      ...
      for (i = 0 ; i < benchWarmupIters + benchIters ; i++) {
        ...
        if(useBufferAPI) {
          MPI.COMM_WORLD.allToAll(sendBuffer, size, MPI.BYTE, 
  ...
  private void runWarmupLoop() throws Exception {
    ...
    for(int i=0 ; i<INITIAL_WARMUP; i++) {
      ...
      if(useBufferAPI) {
        MPI.COMM_WORLD.allToAll(sendBuffer, 1024, MPI.BYTE, 
  ...
  protected void printHeader() {
    System.out.println(OSU_ALLTOALL_TEST);
  ...
  public static void main(String[] args) throws Exception {
    OSUAlltoall reduceTest = new OSUAlltoall(args);
