...
public class OSUReduce extends BenchmarkUtils {
  ...
  public OSUReduce(String[] args) {
    super(args, BenchmarkUtils.OSU_REDUCE_TEST);
    this.args = args;
  }
  ...
  public void runBenchmark() throws Exception {
    ...
    for (size = minMsgSize ; size*FLOAT_SIZE <= maxMsgSize ; size = 2*size) {
      ...
      for (i = 0 ; i < benchWarmupIters + benchIters ; i++) {
        ...
        if(useBufferAPI) {
          MPI.COMM_WORLD.reduce(sendBuffer, recvBuffer, size, 
              MPI.FLOAT, MPI.SUM, root);
        } else { 
          MPI.COMM_WORLD.reduce((float[])sendBuffer, (float[])recvBuffer, size,
              MPI.FLOAT, MPI.SUM, root);
        }
  ...
  private void runWarmupLoop() throws Exception {
    ...
    for(int i=0 ; i<INITIAL_WARMUP; i++) {
      ...
      if(useBufferAPI) {
        MPI.COMM_WORLD.reduce((Buffer)sendBuffer, (Buffer)recvBuffer, 1024, 
            MPI.FLOAT, MPI.SUM, 0);
      } else { 
        MPI.COMM_WORLD.reduce((float[])sendBuffer, (float[])recvBuffer, 1024,
            MPI.FLOAT, MPI.SUM, 0);
      }
  ...
  protected void printHeader() {
    System.out.println(OSU_REDUCE_TEST);
  ...
  public static void main(String[] args) throws Exception {
    OSUReduce reduceTest = new OSUReduce(args);
