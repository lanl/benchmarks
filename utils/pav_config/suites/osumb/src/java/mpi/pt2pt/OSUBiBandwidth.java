...
public class OSUBiBandwidth extends BenchmarkUtils {
  ...
  public OSUBiBandwidth() {

  }

  public OSUBiBandwidth(String[] args) {
    super(args, BenchmarkUtils.OSU_OMPI_BIBW_TEST);
    this.args = args;
  }
	  ...
	  if(me == 0) {
	      ...
	      init = System.nanoTime(); 
            ...
            if(useBufferAPI) {
              ...
              for (int winIdx = 0 ; winIdx < windowSize ; winIdx++) {
                rreqs[winIdx] = MPI.COMM_WORLD.iRecv(
                   (byte[])recvBuffer[winIdx], size, MPI.BYTE, 1, 100);
              } 
            }
            ...
            if(useBufferAPI) {
              ...
              for (int winIdx = 0 ; winIdx < windowSize ; winIdx++) {
                sreqs[winIdx] = MPI.COMM_WORLD.iSend(
                   (byte[])sendBuffer[winIdx], size, MPI.BYTE, 1, 101);
              }
            }
  ...
  protected void printHeader() {
    System.out.println(OSU_OMPI_BIBW_TEST);
    System.out.println("# Size [B]" + "\t\t" + "Bandwidth [MB/s]" + "\t" +
        "Bandwidth [Gb/s]");
  }
  
  public static void main(String[] args) throws Exception {
    OSUBiBandwidth bwTest = new OSUBiBandwidth(args);
